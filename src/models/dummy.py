...
class DummyModel(nn.Module):
    def __init__(
        self, metadata, in_channels_dict: dict, hidden_channels: int, out_channels: int
    ) -> None:
        super().__init__()
        ...
        self.node_proj = nn.ModuleDict(
            {
                ntype: Linear(
                    in_channels_dict.get(ntype, hidden_channels), hidden_channels
                )
        ...
        logger = get_logger()
        logger.info(
            "DummyModel: %d node types, %d edge types",
            len(self.node_types),
            len(self.edge_types),
        )
        logger.info(
            "DummyModel: Outputting predictions for '%s' nodes", self.target_node_type
        )
    ...
    def forward(self, x_dict, edge_index_dict):
        ...
        if self.target_node_type is None or self.target_node_type not in x_dict:
            ...
            out = torch.zeros(
                x_dict[first_ntype].size(0),
                self.out_proj.out_features,
                device=x_dict[first_ntype].device,
            )
