...
DEFAULT_LEARN_WINDOW = (
    10  # number of units to learn from (default: 10 hours when unit=60)
)
...
def parse_fanouts(fanout_str: str):
    ...
    try:
        ...
        raise argparse.ArgumentTypeError(
            "Fanouts must be a comma-separated list of integers"
        ) from exc
...
def parse_split(split_str: str):
    ...
    if len(parts) != 3:
        raise argparse.ArgumentTypeError(
            "Split must have three comma-separated percentages: train,val,test"
        )
    try:
...
def get_args():
    ...
    parser.add_argument(
        "--use_sliding_windows",
        type=str2bool,
        default=DEFAULT_USE_SLIDING_WINDOWS,
    ...
    parser.add_argument(
        ...
        "--neighbor_sampling",
        action="store_true",
        default=DEFAULT_NEIGHBOR_SAMPLING,
    ...
    parser.add_argument(
        ...
        "--neighbor_fanouts",
        type=parse_fanouts,
        default=DEFAULT_NEIGHBOR_FANOUTS,
    ...
    parser.add_argument(
        "--compile_backend",
        type=str,
        default=DEFAULT_COMPILE_BACKEND,
    ...
    if preliminary.config:
        ...
        try:
            ...
            try:
                ...
                raise RuntimeError(
                    "Failed to parse config file. Install pyyaml or provide JSON."
                ) from e

        if not isinstance(cfg, dict):
            raise RuntimeError(
                "Config file must contain a top-level object/dictionary of options"
            )
    ...
    if not args.data_path or os.path.isdir(args.data_path):
        ...
        if files:
            ...
            raise FileNotFoundError(
                f"No CSV files found in {search_dir}. Please provide a dataset or run merge.py."
            )
    else:
        # If a file path was provided, validate it exists
        if not os.path.exists(args.data_path):
            raise FileNotFoundError(
                f"Provided data_path does not exist: {args.data_path}"
            )
