"""Data loading and split/window utilities.

This module provides helpers to load and preprocess the raw flight CSV, compute
chronological splits, and generate sliding window definitions used for training.
Functions are intentionally small and return plain python/numpy structures so
they are easily testable and decoupled from graph construction.
"""
...
from src.utils import get_logger, hhmm_to_minutes
...
def load_data(
    ...
    # Build timestamps
    if "FL_DATE" not in df.columns or not pd.api.types.is_datetime64_any_dtype(
        df["FL_DATE"]
    ):
        df["FL_DATE"] = pd.to_datetime(
            df[["YEAR", "MONTH", "DAY_OF_MONTH"]].rename(
    ...
    logger.info(
        f"Dataset: {len(df)} flights from {df['dep_timestamp'].min()} to {df['dep_timestamp'].max()}"
    )
        ...
        if os.path.isfile(normalize_cols_file):
            try:
                ...
                if num_cols:
                    logger = get_logger()
                    logger.info(
                        f"Normalizing {len(num_cols)} columns using train split statistics"
                    )
...
def compute_splits_from_graph(
    graph, split_ratios: tuple[float, float, float] = (0.8, 0.1, 0.1)
):
    ...
    logger.info(
        f"  Train: [0, {train_end}) = {train_end} flights ({split_ratios[0]*100:.0f}%)"
    )
    logger.info(
        f"  Val: [{train_end}, {val_end}) = {val_end - train_end} flights ({split_ratios[1]*100:.0f}%)"
    )
    logger.info(
        f"  Test: [{val_end}, {n_flights}) = {n_flights - val_end} flights ({split_ratios[2]*100:.0f}%)"
    )
...
def compute_windows_from_graph(graph, unit, learn_window, pred_window, window_stride):
    """Generate sliding window definitions from a graph's timestamps.

    Each window is represented as a dict containing learn_indices and pred_indices
    (compact int indices referring to the chronologically sorted flights in the graph).

    Args:
        graph: HeteroData graph with flight timestamps (either `timestamp_min` or `timestamp`)
        unit: Time unit in minutes used to bucket timestamps
        learn_window: Learning window size in units
        pred_window: Prediction window size in units
        window_stride: Stride between windows in units

    Returns:
        List of window dicts with keys `learn_indices`, `pred_indices`, `time_range`.
    """
    ...
    windows_all = []
    num_possible_windows = max(
        0, (max_unit - min_unit + 1 - total_window_size) // window_stride + 1
    )
    ...
    logger.info(
        f"  Split distribution: train={len(train_windows)}, val={len(val_windows)}, test={len(test_windows)}"
    )
