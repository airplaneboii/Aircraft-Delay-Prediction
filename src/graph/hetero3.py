...
class HeteroGraph3:
    ...
    - Airport: consecutive departures from the same origin airport
    - Airline: consecutive flights operated by the same carrier
    """
    ...
    def build(self):
        ...
        airports = sorted(
            set(self.df["ORIGIN_AIRPORT_ID"]).union(set(self.df["DEST_AIRPORT_ID"]))
        )
        ...
        origin_features = (
            train_df.groupby("ORIGIN_AIRPORT_ID")
            .agg(
                {
            ...
            .rename(columns={"ORIGIN_AIRPORT_ID": "num_of_departures"})
        )  # renaming for clarity

        # features based on destination airport (fit on train only)
        dest_features = (
            train_df.groupby("DEST_AIRPORT_ID")
            .agg(
                {
            ...
            .rename(columns={"DEST_AIRPORT_ID": "num_of_arrivals"})
        )  # renaming for clarity
        ...
        wac_encoder = OneHotEncoder(
            categories=[all_wacs], sparse_output=False, handle_unknown="ignore"
        )
        wac_encoder.fit(
            np.array(all_wacs).reshape(-1, 1)
        )  # fit encoder on all unique WACs
        airport_wac_map = (
        ...
        for airport in airports:
            ...
            dep_row = (
                origin_features.loc[airport]
                if airport in origin_features.index
                else None
            )
            arr_row = (
                dest_features.loc[airport] if airport in dest_features.index else None
            )

            avg_dep_delay = (
                dep_row["DEP_DELAY"] if dep_row is not None else 0
            )  # Average departure delay
            avg_taxi_out = (
                dep_row["TAXI_OUT"] if dep_row is not None else 0
            )  # Average taxi out time
            num_of_departures = (
                dep_row["num_of_departures"] if dep_row is not None else 0
            )  # Number of departures
            num_of_arrivals = (
                arr_row["num_of_arrivals"] if arr_row is not None else 0
            )  # Number of arrivals
            arrivals_avg_distance = (
                arr_row["DISTANCE"] if arr_row is not None else 0
            )  # Average distance of arrivals
            departures_avg_distance = (
                dep_row["DISTANCE"] if dep_row is not None else 0
        ...
        train_airports = sorted(
            set(train_df["ORIGIN_AIRPORT_ID"]).union(set(train_df["DEST_AIRPORT_ID"]))
        )
        train_airport_idx = np.array(
            [airport_map[a] for a in train_airports if a in airport_map], dtype=int
        )
        fit_idx = (
            train_airport_idx
            if len(train_airport_idx) > 0
            else np.arange(len(airport_arr))
        )
        ...
        for tail in aircrafts:
            ...
            features = [avg_dep_delay, avg_distance, crs_elapsed_time, num_of_flights]
        ...
        train_aircraft_idx = np.array(
            [aircraft_map[a] for a in train_aircraft if a in aircraft_map], dtype=int
        )
        fit_idx = (
            train_aircraft_idx
            if len(train_aircraft_idx) > 0
            else np.arange(len(aircraft_arr))
        )
        ...
        airline_grp = (
            train_df.groupby("OP_CARRIER_AIRLINE_ID")
            .agg(
                {
        ...
        train_airline_idx = np.array(
            [airline_map[a] for a in train_airlines if a in airline_map], dtype=int
        )
        fit_idx = (
            train_airline_idx
            if len(train_airline_idx) > 0
            else np.arange(len(airline_arr))
        )
        ...
        flight_features["CRS_DEP_TIME"] = flight_features["CRS_DEP_TIME"].apply(
            hhmm_to_minutes
        )
        flight_features["CRS_ARR_TIME"] = flight_features["CRS_ARR_TIME"].apply(
            hhmm_to_minutes
        )

        # encode time features as cyclical features
        flight_features["dep_time_sin"] = np.sin(
            2 * np.pi * flight_features["CRS_DEP_TIME"] / (24 * 60)
        )
        flight_features["dep_time_cos"] = np.cos(
            2 * np.pi * flight_features["CRS_DEP_TIME"] / (24 * 60)
        )
        flight_features["arr_time_sin"] = np.sin(
            2 * np.pi * flight_features["CRS_ARR_TIME"] / (24 * 60)
        )
        flight_features["arr_time_cos"] = np.cos(
            2 * np.pi * flight_features["CRS_ARR_TIME"] / (24 * 60)
        )

        # encoding day of week
        flight_features["day_sin"] = np.sin(
            2 * np.pi * flight_features["DAY_OF_WEEK"] / 7
        )
        flight_features["day_cos"] = np.cos(
            2 * np.pi * flight_features["DAY_OF_WEEK"] / 7
        )

        # encoding month
        flight_features["month_sin"] = np.sin(
            2 * np.pi * (flight_features["MONTH"] - 1) / 12
        )
        flight_features["month_cos"] = np.cos(
            2 * np.pi * (flight_features["MONTH"] - 1) / 12
        )

        # drop columns that we don't need anymore
        flight_features = flight_features.drop(
            columns=["CRS_DEP_TIME", "CRS_ARR_TIME", "DAY_OF_WEEK", "MONTH"]
        )
        ...
        self.df["dep_timestamp"] = self.df["FL_DATE"] + pd.to_timedelta(
            self.df["dep_minutes_calc"], unit="m"
        )
        ...
        if timestamp_range > 0:
            self.df["dep_timestamp_norm"] = (
                (self.df["dep_timestamp"] - min_timestamp).dt.total_seconds()
                / timestamp_range
            ).astype(np.float32)
        else:
            self.df["dep_timestamp_norm"] = 0.0

        self.df["dep_timestamp_minutes"] = (
            (self.df["dep_timestamp"] - min_timestamp).dt.total_seconds() / 60.0
        ).astype(np.float32)

        data["flight"].timestamp = torch.tensor(
            self.df["dep_timestamp_norm"].values, dtype=torch.float32
        )
        data["flight"].timestamp_min = torch.tensor(
            self.df["dep_timestamp_minutes"].values, dtype=torch.float32
        )
        ...
        data["flight", "originates_from", "airport"].edge_index = torch.tensor(
            [flight_index, origin], dtype=torch.long
        )
        # Reverse edge: airport has departing flights
        data["airport", "has_departure", "flight"].edge_index = torch.tensor(
            [origin, flight_index], dtype=torch.long
        )

        # Edge 2: flight arrives at airport
        data["flight", "arrives_at", "airport"].edge_index = torch.tensor(
            [flight_index, dest], dtype=torch.long
        )
        # Reverse edge: airport has arriving flights
        data["airport", "has_arrival", "flight"].edge_index = torch.tensor(
            [dest, flight_index], dtype=torch.long
        )

        # Edge 3: flight operated by airline
        data["flight", "operated_by", "airline"].edge_index = torch.tensor(
            [flight_index, airline], dtype=torch.long
        )
        # Reverse edge: airline operates flights
        data["airline", "operates", "flight"].edge_index = torch.tensor(
            [airline, flight_index], dtype=torch.long
        )

        # Edge 4: flight performed by aircraft
        data["flight", "performed_by", "aircraft"].edge_index = torch.tensor(
            [flight_index, aircraft], dtype=torch.long
        )
        # Reverse edge: aircraft performs flights
        data["aircraft", "performs", "flight"].edge_index = torch.tensor(
            [aircraft, flight_index], dtype=torch.long
        )

        # Temporal edges: link consecutive flights
        # Prepare temporal index column
        self.df["DEP_TIME_MINUTES"] = self.df["CRS_DEP_TIME"].apply(hhmm_to_minutes)
        df_sorted = self.df.sort_values(["FL_DATE", "DEP_TIME_MINUTES"]).reset_index(
            drop=True
        )

        # Edge 5: next_same_aircraft - consecutive flights by the same aircraft
        aircraft_next_src = []
        aircraft_next_dst = []
        df_aircraft_sorted = df_sorted.sort_values(
            ["TAIL_NUM", "FL_DATE", "DEP_TIME_MINUTES"]
        ).reset_index(drop=True)
        for tail, group in df_aircraft_sorted.groupby("TAIL_NUM"):
            idx_list = group.index.tolist()
            for i in range(len(idx_list) - 1):
                aircraft_next_src.append(idx_list[i])
                aircraft_next_dst.append(idx_list[i + 1])

        if aircraft_next_src:
            data["flight", "next_same_aircraft", "flight"].edge_index = torch.tensor(
                [aircraft_next_src, aircraft_next_dst], dtype=torch.long
            )

        # Edge 6: next_same_origin - consecutive departures from the same origin airport
        airport_next_src = []
        airport_next_dst = []
        df_airport_sorted = df_sorted.sort_values(
            ["ORIGIN_AIRPORT_ID", "FL_DATE", "DEP_TIME_MINUTES"]
        ).reset_index(drop=True)
        for origin_id, group in df_airport_sorted.groupby("ORIGIN_AIRPORT_ID"):
            idx_list = group.index.tolist()
            for i in range(len(idx_list) - 1):
                airport_next_src.append(idx_list[i])
                airport_next_dst.append(idx_list[i + 1])

        if airport_next_src:
            data["flight", "next_same_origin", "flight"].edge_index = torch.tensor(
                [airport_next_src, airport_next_dst], dtype=torch.long
            )

        # Edge 7: next_same_airline - consecutive flights operated by the same carrier
        airline_next_src = []
        airline_next_dst = []
        df_airline_sorted = df_sorted.sort_values(
            ["OP_CARRIER_AIRLINE_ID", "FL_DATE", "DEP_TIME_MINUTES"]
        ).reset_index(drop=True)
        for carrier_id, group in df_airline_sorted.groupby("OP_CARRIER_AIRLINE_ID"):
            idx_list = group.index.tolist()
            for i in range(len(idx_list) - 1):
                airline_next_src.append(idx_list[i])
                airline_next_dst.append(idx_list[i + 1])

        if airline_next_src:
            data["flight", "next_same_airline", "flight"].edge_index = torch.tensor(
                [airline_next_src, airline_next_dst], dtype=torch.long
            )
        ...
        data["flight"].y_reg = torch.tensor(
            arr_delay_vals, dtype=torch.float32
        ).unsqueeze(1)
        ...
        if "ARR_DEL15" in self.df.columns:
            ...
            cls_vals = (
                (self.df["ARR_DELAY"].fillna(0.0) >= border * 60).astype(int).values
            )
