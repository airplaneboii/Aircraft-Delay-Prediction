...
class HeteroGraph5:
    ...
    def build(self):
        ...
        if (
            "FL_DATE" not in self.df.columns
            or not pd.api.types.is_datetime64_any_dtype(self.df["FL_DATE"])
        ):
        ...
        airport_origin_counts = pd.Series(
            {a: origin_counts.get(a, 0) for a in airports}
        )
        airport_dest_counts = pd.Series({a: dest_counts.get(a, 0) for a in airports})

        # Normalize counts
        max_origin = (
            airport_origin_counts.max() if airport_origin_counts.max() > 0 else 1
        )
        ...
        airport_info = self.df[
            ["ORIGIN_AIRPORT_ID", "ORIGIN_CITY_MARKET_ID", "ORIGIN_WAC"]
        ].drop_duplicates(subset=["ORIGIN_AIRPORT_ID"])
        airport_info_dict = {
            row["ORIGIN_AIRPORT_ID"]: row for _, row in airport_info.iterrows()
        }

        dest_info = self.df[
            ["DEST_AIRPORT_ID", "DEST_CITY_MARKET_ID", "DEST_WAC"]
        ].drop_duplicates(subset=["DEST_AIRPORT_ID"])
        dest_info_dict = {
            row["DEST_AIRPORT_ID"]: row for _, row in dest_info.iterrows()
        }
        airport_info_dict.update(dest_info_dict)

        all_city_markets = sorted(
            set(self.df["ORIGIN_CITY_MARKET_ID"]).union(
                set(self.df["DEST_CITY_MARKET_ID"])
            )
        )
        ...
        for a in airports:
            ...
            city_id = info.get(
                ...
                info.get("DEST_CITY_MARKET_ID", all_city_markets[0]),
            )
            wac_id = info.get("ORIGIN_WAC", info.get("DEST_WAC", all_wac[0]))
            city_ids.append(city_id)
            wac_ids.append(wac_id)
        city_arr = city_encoder.transform(np.array(city_ids).reshape(-1, 1)).astype(
            np.float32
        )
        wac_arr = wac_encoder.transform(np.array(wac_ids).reshape(-1, 1)).astype(
            np.float32
        )

        # Base numeric features (normalized counts)
        base_counts = np.stack(
            [
                airport_origin_norm.values.astype(np.float32),
                airport_dest_norm.values.astype(np.float32),
            ],
            axis=1,
        )
        ...
        self.df["dep_timestamp"] = self.df["FL_DATE"] + pd.to_timedelta(
            self.df["dep_minutes"], unit="m"
        )
        self.df["arr_timestamp"] = self.df["FL_DATE"] + pd.to_timedelta(
            self.df["arr_minutes"], unit="m"
        )
        ...
        dep_time_sin = np.sin(2 * np.pi * self.df["dep_minutes"] / (24 * 60)).astype(
            np.float32
        )
        dep_time_cos = np.cos(2 * np.pi * self.df["dep_minutes"] / (24 * 60)).astype(
            np.float32
        )
        arr_time_sin = np.sin(2 * np.pi * self.df["arr_minutes"] / (24 * 60)).astype(
            np.float32
        )
        arr_time_cos = np.cos(2 * np.pi * self.df["arr_minutes"] / (24 * 60)).astype(
            np.float32
        )
        ...
        crs_elapsed = (
            self.df["CRS_ELAPSED_TIME"].fillna(0.0).to_numpy(dtype=np.float32) / 600.0
        )
        distance_norm = (
            self.df["DISTANCE"].fillna(0.0).to_numpy(dtype=np.float32) / 5000.0
        )
        ...
        data["flight"].timestamp_min = torch.from_numpy(
            self.df["dep_timestamp_minutes"].to_numpy(dtype=np.float32)
        ).float()
        ...
        data["flight", "originates_from", "airport"].edge_index = torch.tensor(
            origin_edges, dtype=torch.long
        )
        data["airport", "has_departure", "flight"].edge_index = torch.tensor(
            origin_edges[[1, 0], :], dtype=torch.long
        )
        ...
        data["flight", "arrives_at", "airport"].edge_index = torch.tensor(
            dest_edges, dtype=torch.long
        )
        data["airport", "has_arrival", "flight"].edge_index = torch.tensor(
            dest_edges[[1, 0], :], dtype=torch.long
        )
        ...
        data["flight", "operated_by", "airline"].edge_index = torch.tensor(
            airline_edges, dtype=torch.long
        )
        data["airline", "operates", "flight"].edge_index = torch.tensor(
            airline_edges[[1, 0], :], dtype=torch.long
        )
        ...
        data["flight", "performed_by", "aircraft"].edge_index = torch.tensor(
            aircraft_edges, dtype=torch.long
        )
        data["aircraft", "performs", "flight"].edge_index = torch.tensor(
            aircraft_edges[[1, 0], :], dtype=torch.long
        )
        ...
        if aircraft_next:
            next_aircraft_edges = (
                torch.tensor(np.concatenate(aircraft_next, axis=0), dtype=torch.long)
                .t()
                .contiguous()
            )
            data["flight", "next_same_aircraft", "flight"].edge_index = (
                next_aircraft_edges
            )
        ...
        if origin_next:
            next_origin_edges = (
                torch.tensor(np.concatenate(origin_next, axis=0), dtype=torch.long)
                .t()
                .contiguous()
            )
        ...
        if airline_next:
            next_airline_edges = (
                torch.tensor(np.concatenate(airline_next, axis=0), dtype=torch.long)
                .t()
                .contiguous()
            )
            data["flight", "next_same_airline", "flight"].edge_index = (
                next_airline_edges
            )
