...
def main():
    ...
    if args.mode == "split":
        if not args.file:
            print("Error: --file is required in split mode.")
            return
        ...
        if args.list:
            try:
                ...
                print("Error: --list must be a valid Python list string.")
                return
        elif args.file:
            items = split_file_to_list(args.file)
        else:
            print("Error: Either --list or --file must be provided in format mode.")
            return
